...
# Velocity metric ---------------------------------------------------------
get_forward_velocity <- function(sp){
 sp <- species[1] # use for testing
  ...
  flsFut <- list.files(flsFut, pattern = '.tif', full.names = TRUE)
  rcp <- c('_45_', '_85_')
  ...
  rsltdo <- map(.x = 1:length(rcp), function(k){
    ...
    rs <- map(.x = 1:length(yrs), .f = function(i){
      ...
      flsPres <- list.files(dirPres, pattern = '_range_masked.tif', full.names = TRUE)
      ...
      flesFut <- grep(rcp[k], flesFut, value = TRUE)
       ...
       vel <- map(.x = 1:length(gcms), .f = function(j){
        ...
        out <- glue('./outputs/velocity/generalist/forwardVel')
        ifelse(!file.exists(out), dir_create(out), print('Already exists'))
        terra::writeRaster(velref, glue('{out}/{sp}_forwardVel{rcp[k]}_{yrs[i]}.tif'),
  ...
  ## obtain mean for the reference stack
    ...
    ifelse(!file.exists(out), dir_create(out), print('Already exists'))
    terra::writeRaster(ftr.mean, glue('{out}/{sp}_forwardRefugia{rcp[k]}{yrs[i]}.tif'),
...
map(species,get_backward_velocity)
